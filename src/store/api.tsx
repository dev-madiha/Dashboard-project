...
const EditPatient = () => {
  const { patient_id } = useParams(); // if route has param like /edit-patient/:patient_id
  const location = useLocation();
  const navigate = useNavigate();

  // If patient was passed via navigate({ state: patient })
  ...
  // optional: fetch latest patient data if not passed in state
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.put(`${API_BASE_URL}/patients/${patient?.id || patient_id}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });

      toast.success("Patient updated successfully!");
      setTimeout(() => navigate("/admin/patient"), 1500);
    } catch (err: any) {
